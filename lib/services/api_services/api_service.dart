...
class ApiService {
  ...
  static Future<dynamic> get(String endpoint, dynamic body) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${ApiConstant.apiBaseUrl}$endpoint'),
        headers: headers,
      );
  ...
  static Future<dynamic> post(String endpoint, dynamic body, {bool isLogin = false}) async {
    try {
      // Prepare headers with authorization
      Map<String, String> headers = _getHeadersWithAuth(body);
      ...
      // Choose base URL
      final baseUrl = isLogin ? ApiConstant.loginBaseUrl : ApiConstant.apiBaseUrl;
